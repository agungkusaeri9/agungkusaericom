    ...
    <div class="grid grid-cols-1">
        <div class="px-4">
            <div class="md:flex md:justify-end">
                <form class="w-full mb-4 md:w-[520px]" action="{{ route('posts.search') }}">
                    <div class="relative">
                        <input type="text"
                            class="w-full px-4 py-2 pr-10 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                            placeholder="Search keyword article..." name="q" value="{{ request('q') }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 md:gap-6">
                @foreach ($posts as $post)
                    <x-Frontend.CardPost title="{{ $post->title }}" image="{{ $post->image() }}"
                        metadescription="{{ $post->meta_description }}" slug="{{ $post->slug }}"

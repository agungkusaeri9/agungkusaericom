        ...
        <div class="bg-white mb-5 rounded-md">
            <a href="{{ route('projects.show', $latest->slug) }}">
                <img src="{{ $latest->image() }}" alt="{{ $latest->name }}"
                    class="aspect-video h-30 rounded-t-md w-full md:h-[150px] md:w-full object-cover hover:brightness-50">
                <div class="pt-2 px-1">
                    <h1 class="text-xs font-semibold text-slate-800 mb-2 md:text-lg text-justify">{{ $latest->name }}
                    </h1>
